...
import type { EducationLevel } from "../../types/educations"
import { EducationIcon } from "./education-level-icon"

export function EducationLevelItem({ level }: { level: EducationLevel }) {
  const { start, end } = level.formationPeriod
  const isOngoing = !end

  return (
    <CollapsibleWithContext
      defaultOpen={level.isExpanded}
        ...
        >
          <div className="relative z-1 mb-1 flex items-center gap-3">
            <div
              ...
              aria-hidden
            >
              <EducationIcon className="size-4" icon={level.icon} />
            </div>

            <h4 className="flex-1 font-medium text-balance">{level.title}</h4>
